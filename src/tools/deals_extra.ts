...
export function registerDealExtraTools(server: McpServer, client: PipedriveClient): void {

  // ── DEALS SUMMARY ───────────────────────────────────────────────────────────
  server.registerTool("pipedrive_get_deals_summary", {
...
Filter with same params as list_deals: user_id, pipeline_id, filter_id, stage_id, status.`,
    inputSchema: z.object({
      user_id: z.number().int().positive().optional().describe("Filter by owner user ID"),
      pipeline_id: z.number().int().positive().optional().describe("Filter by pipeline ID"),
      filter_id: z.number().int().positive().optional().describe("Filter by saved filter ID"),
      stage_id: z.number().int().positive().optional().describe("Filter by stage ID"),
      status: z.string().optional().describe("open | won | lost (comma-separated)"),
    }).strict(),
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
  }, async (params) => {
    try {
      const data = await client.get<SingleResponse<unknown>>("/api/v2/deals/summary", compactBody(params as Record<string, unknown>));
      return ok(serialize(data.data));
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_get_archived_deals_summary", {
    ...
    try {
      const data = await client.get<SingleResponse<unknown>>("/api/v2/deals/archived/summary", compactBody(params as Record<string, unknown>));
      return ok(serialize(data.data));
    } catch (e) { return err(e); }
  });

  // ── DEALS TIMELINE ──────────────────────────────────────────────────────────
  server.registerTool("pipedrive_get_deals_timeline", {
...
→ returns deals grouped by month for the year 2025, plotted by their won_time.`,
    inputSchema: z.object({
      start_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).describe("Start date YYYY-MM-DD (required)"),
      interval: z.enum(["day", "week", "month", "quarter"]).describe("Time interval (required)"),
      amount: z.number().int().min(1).max(365).describe("Number of intervals from start_date (required)"),
      field_key: z.string().min(1).describe("Date field key to group by (required). e.g. 'add_time', 'won_time', 'expected_close_date'"),
      user_id: z.number().int().positive().optional().describe("Filter by owner"),
      pipeline_id: z.number().int().positive().optional(),
      filter_id: z.number().int().positive().optional(),
      exclude_deals: z.number().int().min(0).max(1).optional().describe("1=only counts (no deal objects), 0=include deal data"),
      totals_convert_currency: z.string().length(3).optional().describe("3-letter currency code to convert totals to"),
    }).strict(),
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
  }, async (params) => {
    try {
      const data = await client.get<SingleResponse<unknown>>("/api/v2/deals/timeline", compactBody(params as Record<string, unknown>));
      return ok(serialize(data.data));
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_get_archived_deals_timeline", {
    ...
    inputSchema: z.object({
      start_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).describe("Start date YYYY-MM-DD (required)"),
      interval: z.enum(["day", "week", "month", "quarter"]).describe("Time interval (required)"),
      amount: z.number().int().min(1).max(365).describe("Number of intervals (required)"),
    ...
    try {
      const data = await client.get<SingleResponse<unknown>>("/api/v2/deals/archived/timeline", compactBody(params as Record<string, unknown>));
      return ok(serialize(data.data));
    } catch (e) { return err(e); }
  });

  // ── DEAL DISCOUNTS ──────────────────────────────────────────────────────────
  server.registerTool("pipedrive_list_deal_discounts", {
...
Returns: id, type (percentage | amount), value, description.`,
    ...
    try {
      const data = await client.get<{ success: boolean; data: unknown[] }>(`/api/v2/deals/${deal_id}/discounts`);
      return ok(`${data.data?.length ?? 0} discount(s).\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_add_deal_discount", {
...
value: the discount value (% if percentage, absolute if amount).`,
    inputSchema: z.object({
      deal_id: z.number().int().positive().describe("Deal ID"),
      type: z.enum(["percentage", "amount"]).describe("Discount type (required)"),
    ...
    try {
      const data = await client.post<SingleResponse<unknown>>(`/api/v2/deals/${deal_id}/discounts`, compactBody(body as Record<string, unknown>));
      return ok(`Discount added to deal ${deal_id}.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_update_deal_discount", {
    ...
    try {
      const data = await client.patch<SingleResponse<unknown>>(`/api/v2/deals/${deal_id}/discounts/${discount_id}`, compactBody(body as Record<string, unknown>));
      return ok(`Discount ${discount_id} updated.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_delete_deal_discount", {
  ...
  server.registerTool("pipedrive_list_deal_installments", {
...
Provide deal_ids (comma-separated, up to 100) for bulk fetching.`,
    ...
    try {
      const data = await client.get<ListResponseV2<unknown>>("/api/v2/deals/installments", compactBody(params as Record<string, unknown>));
      const nc = data.additional_data?.next_cursor;
      return ok(`${data.data.length} installment(s).${nc ? ` Next cursor: ${nc}` : ""}\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_add_deal_installment", {
...
If the deal has any recurring products, installments are NOT allowed.`,
    inputSchema: z.object({
      ...
      amount: z.number().positive().describe("Installment amount — must be > 0 (required)"),
      billing_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).describe("Billing date YYYY-MM-DD (required)"),
    }).strict(),
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
  }, async ({ deal_id, ...body }) => {
    try {
      const data = await client.post<SingleResponse<unknown>>(`/api/v2/deals/${deal_id}/installments`, compactBody(body as Record<string, unknown>));
      return ok(`Installment added to deal ${deal_id}.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_update_deal_installment", {
    title: "Update Deal Installment",
    description: `Update an installment on a deal via PATCH (v2 API). Growth+ plans only.`,
    inputSchema: z.object({
      deal_id: z.number().int().positive().describe("Deal ID"),
      installment_id: z.number().int().positive().describe("Installment ID"),
    ...
    try {
      const data = await client.patch<SingleResponse<unknown>>(`/api/v2/deals/${deal_id}/installments/${installment_id}`, compactBody(body as Record<string, unknown>));
      return ok(`Installment ${installment_id} updated.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_delete_deal_installment", {
    ...
    try {
      await client.del(`/api/v2/deals/${deal_id}/installments/${installment_id}`);
      return ok(`Installment ${installment_id} removed from deal ${deal_id}.`);
    } catch (e) { return err(e); }
  });

  // ── DEAL CHANGELOG ──────────────────────────────────────────────────────────
  server.registerTool("pipedrive_list_deal_changelog", {
...
Useful for auditing who changed what and when.`,
    ...
    try {
      const data = await client.get<{ success: boolean; data: unknown[]; additional_data?: { next_cursor?: string } }>(
        `/api/v1/deals/${deal_id}/changelog`, compactBody({ cursor, limit }));
      const nc = data.additional_data?.next_cursor;
  ...
  // ── DEAL FILES ──────────────────────────────────────────────────────────────
  server.registerTool("pipedrive_list_deal_files", {
...
Alternative: use pipedrive_list_files with deal_id param.`,
    ...
    try {
      const data = await client.get<ListResponseV1<unknown>>(
        `/api/v1/deals/${deal_id}/files`, compactBody({ start, limit, sort }));
  ...
  // ── DEAL FLOW / UPDATES ─────────────────────────────────────────────────────
  server.registerTool("pipedrive_list_deal_updates", {
...
items: comma-separated filter (activity, plannedActivity, note, file, change, deal, follower, participant, mailMessage, mailMessageWithAttachment, invoice, activityFile, document).`,
    ...
    try {
      const data = await client.get<ListResponseV1<unknown>>(
        `/api/v1/deals/${deal_id}/flow`, compactBody(params as Record<string, unknown>));
      const pg = data.additional_data?.pagination;
  ...
  // ── DEAL MAIL MESSAGES ──────────────────────────────────────────────────────
  server.registerTool("pipedrive_list_deal_mail_messages", {
...
Returns mail threads linked to this deal via Smart BCC or email sync.`,
    ...
    try {
      const data = await client.get<ListResponseV1<unknown>>(
        `/api/v1/deals/${deal_id}/mailMessages`, compactBody({ start, limit }));
      const pg = data.additional_data?.pagination;
  ...
  server.registerTool("pipedrive_list_deal_permitted_users", {
