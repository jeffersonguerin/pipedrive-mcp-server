...
export function registerActivityTools(server: McpServer, client: PipedriveClient): void {
  ...
  server.registerTool("pipedrive_list_activities", {
...
busy (was busy_flag), location is nested object, is_deleted replaces is_active.`,
    inputSchema: z.object({
      filter_id: z.number().int().positive().optional().describe("Use a saved Pipedrive filter"),
      ids: z.string().optional().describe("Comma-separated activity IDs (up to 100); ignored if filter_id set"),
      owner_id: z.number().int().positive().optional().describe("Filter by owner user ID"),
      person_id: z.number().int().positive().optional().describe("Filter by primary participant person ID"),
      deal_id: z.number().int().positive().optional(),
      lead_id: z.string().optional().describe("Filter by lead ID (UUID string)"),
      org_id: z.number().int().positive().optional(),
      done: z.boolean().optional().describe("true=completed, false=open/pending"),
      updated_since: z.string().optional().describe("RFC3339 datetime: return activities updated at or after this time (e.g. 2025-01-01T10:20:00Z)"),
      updated_until: z.string().optional().describe("RFC3339 datetime: return activities updated before this time"),
      cursor: z.string().optional(),
      limit: z.number().int().min(1).max(500).default(50),
      sort_by: z.enum(["id","add_time","update_time","due_date"]).default("id").describe("Sort field (default: id, as per Pipedrive API default)"),
    ...
    try {
      const data = await client.get<ListResponseV2<unknown>>("/api/v2/activities", compactBody(params as Record<string, unknown>));
      const nc = data.additional_data?.next_cursor;
      return ok(`${data.data.length} activity/ies.${nc ? ` Next cursor: ${nc}` : ""}\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_get_activity", {
    title: "Get Activity",
    ...
    try {
      const data = await client.get<SingleResponse<unknown>>(`/api/v2/activities/${id}`, compactBody({ include_fields }));
      return ok(serialize(data.data));
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_create_activity", {
...
project_id: link to a Pipedrive Projects entity.`,
    inputSchema: z.object({
      ...
      type: z.string().min(1).describe("Activity type key_string (required). Use pipedrive_list_activity_types to list valid values."),
      ...
      duration: z.string().regex(/^\d{2}:\d{2}$/).optional().describe("HH:MM"),
      owner_id: z.number().int().positive().optional().describe("Owner user ID (was user_id in v1)"),
      deal_id: z.number().int().positive().optional(),
      lead_id: z.string().optional().describe("Lead ID (UUID)"),
      person_id: z.number().int().positive().optional().describe("Person ID (can also be set via participants array)"),
      org_id: z.number().int().positive().optional(),
      project_id: z.number().int().positive().optional().describe("Pipedrive Project ID"),
      note: z.string().optional().describe("Activity note (HTML allowed, max 100KB since Sep 2025)"),
      public_description: z.string().optional(),
      done: z.boolean().optional().describe("Mark as completed on creation"),
      busy: z.boolean().optional().describe("Mark time as busy in calendar (was busy_flag in v1)"),
      private: z.boolean().optional(),
      priority: z.number().int().optional().describe("Priority integer (map via pipedrive_list_activity_fields)"),
      participants: z.array(z.object({
        person_id: z.number().int().positive(),
        primary: z.boolean().optional().describe("Primary participant (also sets activity.person_id)"),
      }).strict()).optional().describe("Activity participants"),
      attendees: z.array(z.object({
        email_address: z.string().describe("Attendee email address (string, not validated as email to handle all Pipedrive formats)"),
        name: z.string().optional(),
        status: z.string().optional().describe("accepted | declined | tentative"),
        person_id: z.number().int().positive().optional(),
        user_id: z.number().int().positive().optional(),
      }).strict()).optional().describe("Calendar attendees with email addresses"),
      location: z.object({
        value: z.string().describe("Address string"),
      }).passthrough().optional().describe("Location object (was flat location_* fields in v1)"),
    }).strict(),
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
  }, async (params) => {
    try {
      const data = await client.post<SingleResponse<unknown>>("/api/v2/activities", compactBody(params as Record<string, unknown>));
      return ok(`Activity created.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_update_activity", {
...
person_id can be set directly or via participants array (both work in v2).`,
    inputSchema: z.object({
      ...
      person_id: z.number().int().positive().optional().describe("Set person directly or via participants"),
      org_id: z.number().int().positive().optional(),
    ...
    try {
      const data = await client.patch<SingleResponse<unknown>>(`/api/v2/activities/${id}`, compactBody(fields as Record<string, unknown>));
      return ok(`Activity ${id} updated.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_delete_activity", {
    ...
    try {
      ...
      return ok(`Activity ${id} deleted (soft). Permanently removed after 30 days.`);
    } catch (e) { return err(e); }
  });
}
