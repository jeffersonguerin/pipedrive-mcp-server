...
const PhoneEmailSchema = z.object({
  ...
  label: z.string().optional().describe("Label: work | home | mobile | other (default: work)"),
...
export function registerPersonTools(server: McpServer, client: PipedriveClient): void {
  ...
  server.registerTool("pipedrive_list_persons", {
    title: "List Persons",
...
v2: phones/emails are renamed arrays; is_deleted replaces active_flag (negated).`,
    inputSchema: z.object({
      ...
      ids: z.string().optional().describe("Comma-separated person IDs (up to 100)"),
      owner_id: z.number().int().positive().optional(),
      org_id: z.number().int().positive().optional().describe("Filter by organization ID"),
      name: z.string().optional().describe("Filter persons by name (partial match)"),
      deal_id: z.number().int().positive().optional().describe("Filter by linked deal ID"),
      updated_since: z.string().optional().describe("RFC3339: persons updated at or after (e.g. 2025-01-01T10:20:00Z)"),
      updated_until: z.string().optional().describe("RFC3339: persons updated before"),
      cursor: z.string().optional(),
      limit: z.number().int().min(1).max(500).default(50),
      sort_by: z.enum(["id","add_time","update_time"]).default("add_time").describe("Sort field (v2 supports: id, add_time, update_time)"),
      sort_direction: z.enum(["asc","desc"]).default("desc"),
      include_fields: z.string().optional().describe("Comma-separated optional fields (see get_person for list)"),
      custom_fields: z.string().optional().describe("Comma-separated custom field keys to include (max 15)"),
    }).strict(),
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
  }, async (params) => {
    try {
      const data = await client.get<ListResponseV2<unknown>>("/api/v2/persons", compactBody(params as Record<string, unknown>));
      const nc = data.additional_data?.next_cursor;
      return ok(`${data.data.length} person(s).${nc ? ` Next cursor: ${nc}` : ""}\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_get_person", {
    title: "Get Person",
...
custom_fields: comma-separated keys to include (max 15).`,
    ...
    try {
      const data = await client.get<SingleResponse<unknown>>(`/api/v2/persons/${id}`, compactBody({ include_fields, custom_fields }));
      return ok(serialize(data.data));
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_create_person", {
    title: "Create Person",
...
Name rules: provide name OR (first_name + last_name). Both can't be empty strings.`,
    inputSchema: z.object({
      name: z.string().min(1).optional().describe("Full name (or provide first_name + last_name)"),
      first_name: z.string().optional(),
      last_name: z.string().optional(),
      org_id: z.number().int().positive().optional(),
      owner_id: z.number().int().positive().optional(),
      phones: z.array(PhoneEmailSchema).optional().describe("Phone numbers"),
      emails: z.array(PhoneEmailSchema).optional().describe("Email addresses"),
      label_ids: z.array(z.number().int()).optional(),
      visible_to: z.number().int().optional().describe("Visibility: 1, 3, 5, or 7"),
      marketing_status: z.enum(["no_consent","unsubscribed","subscribed","archived"]).optional().describe("Marketing consent status (requires Campaigns product)"),
      custom_fields: z.record(z.unknown()).optional(),
    }).strict(),
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
  }, async (params) => {
    try {
      const data = await client.post<SingleResponse<unknown>>("/api/v2/persons", compactBody(params as Record<string, unknown>));
      return ok(`Person created.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_update_person", {
    title: "Update Person",
...
marketing_status can only be changed once from old to new value.`,
    inputSchema: z.object({
      ...
      org_id: z.number().int().positive().optional(),
      owner_id: z.number().int().positive().optional(),
      phones: z.array(PhoneEmailSchema).optional().describe("Replaces existing phones array"),
      emails: z.array(PhoneEmailSchema).optional().describe("Replaces existing emails array"),
      label_ids: z.array(z.number().int()).optional(),
      visible_to: z.number().int().optional(),
      marketing_status: z.enum(["no_consent","unsubscribed","subscribed","archived"]).optional().describe("Marketing consent status (can only change once)"),
      custom_fields: z.record(z.unknown()).optional(),
    }).strict(),
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
  }, async ({ id, ...fields }) => {
    try {
      const data = await client.patch<SingleResponse<unknown>>(`/api/v2/persons/${id}`, compactBody(fields as Record<string, unknown>));
      return ok(`Person ${id} updated.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_delete_person", {
    title: "Delete Person",
  ...
  server.registerTool("pipedrive_search_persons", {
...
IMPORTANT: Use organization_id (not org_id) to filter by organization.`,
    inputSchema: z.object({
      ...
      fields: z.string().optional().describe("Fields: name, email, phone, notes, custom_fields"),
      exact_match: z.boolean().optional(),
      organization_id: z.number().int().positive().optional().describe("Filter by organization ID (note: organization_id, not org_id)"),
      include_fields: z.string().optional().describe("Optional: person.picture"),
      cursor: z.string().optional(),
      limit: z.number().int().min(1).max(500).default(50),
    }).strict(),
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    ...
    try {
      const data = await client.get<ListResponseV2<unknown>>("/api/v2/persons/search", compactBody(params as Record<string, unknown>));
      const nc = data.additional_data?.next_cursor;
      return ok(`${data.data.length} person(s) for "${params.term}".${nc ? ` Next cursor: ${nc}` : ""}\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_merge_persons", {
...
All deals, activities, notes of the merged person will be transferred to merge_with_id.`,
    inputSchema: z.object({
      id: z.number().int().positive().describe("Person ID that will be merged and deleted"),
      merge_with_id: z.number().int().positive().describe("Person ID to keep (data from this person is prioritized)"),
    }).strict(),
    annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: true },
  }, async ({ id, merge_with_id }) => {
    try {
      const data = await client.put<SingleResponse<unknown>>(`/api/v1/persons/${id}/merge`, { merge_with_id });
      return ok(`Person ${id} merged into person ${merge_with_id}.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_list_person_followers", {
...
Returns: user_id, add_time (RFC3339).`,
    ...
    try {
      const data = await client.get<ListResponseV2<unknown>>(`/api/v2/persons/${person_id}/followers`, compactBody({ cursor, limit }));
      const nc = data.additional_data?.next_cursor;
      return ok(`${data.data.length} follower(s).${nc ? ` Next cursor: ${nc}` : ""}\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_add_person_follower", {
    ...
    try {
      const data = await client.post<SingleResponse<unknown>>(`/api/v2/persons/${person_id}/followers`, { user_id });
      return ok(`User ${user_id} added as follower.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_delete_person_follower", {
    ...
    try {
      ...
      return ok(`User ${user_id} removed as follower from person ${person_id}.`);
    } catch (e) { return err(e); }
  });
}
