...
export function registerDealTools(server: McpServer, client: PipedriveClient): void {
  ...
  // ── LIST DEALS ──────────────────────────────────────────────────────────────
  server.registerTool("pipedrive_list_deals", {
    title: "List Deals",
...
Pagination: pass additional_data.next_cursor as cursor. Max limit: 500.`,
    inputSchema: z.object({
      filter_id: z.number().int().positive().optional().describe("Saved Pipedrive filter ID (overrides other filters)"),
      ids: z.string().optional().describe("Comma-separated deal IDs (up to 100); ignored if filter_id is set"),
      person_id: z.number().int().positive().optional().describe("Filter by linked person ID"),
      org_id: z.number().int().positive().optional().describe("Filter by linked organization ID"),
      pipeline_id: z.number().int().positive().optional().describe("Filter by pipeline ID"),
      stage_id: z.number().int().positive().optional().describe("Filter by stage ID"),
      owner_id: z.number().int().positive().optional().describe("Filter by owner user ID"),
      status: z.string().optional().describe("Filter: open | won | lost | deleted (comma-separated)"),
      updated_since: z.string().optional().describe("RFC3339: deals updated at or after this time (e.g. 2025-01-01T10:20:00Z)"),
      updated_until: z.string().optional().describe("RFC3339: deals updated before this time"),
      cursor: z.string().optional().describe("Cursor from previous response additional_data.next_cursor"),
      limit: z.number().int().min(1).max(500).default(50).describe("Results per page (default 50, max 500)"),
      sort_by: z.enum(["id","add_time","update_time"]).default("add_time").describe("Sort field (v2 supports: id, add_time, update_time only)"),
      sort_direction: z.enum(["asc","desc"]).default("desc"),
      include_fields: z.string().optional().describe("Comma-separated optional fields (see description)"),
      custom_fields: z.string().optional().describe("Comma-separated custom field keys to include (max 15)"),
    }).strict(),
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
  }, async (params) => {
    try {
      const data = await client.get<ListResponseV2<unknown>>("/api/v2/deals", compactBody(params as Record<string, unknown>));
      const nc = data.additional_data?.next_cursor;
      return ok(`${data.data.length} deal(s).${nc ? ` Next cursor: ${nc}` : " (end)"}\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  // ── GET DEAL ────────────────────────────────────────────────────────────────
  server.registerTool("pipedrive_get_deal", {
    title: "Get Deal",
...
custom_fields: comma-separated keys to include (max 15 keys, reduces response size).`,
    inputSchema: z.object({
      ...
      include_fields: z.string().optional().describe("Comma-separated optional fields"),
      custom_fields: z.string().optional().describe("Comma-separated custom field keys (max 15)"),
    }).strict(),
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
  }, async ({ id, include_fields, custom_fields }) => {
    try {
      const data = await client.get<SingleResponse<unknown>>(`/api/v2/deals/${id}`, compactBody({ include_fields, custom_fields }));
      return ok(serialize(data.data));
    } catch (e) { return err(e); }
  });

  // ── CREATE DEAL ─────────────────────────────────────────────────────────────
  server.registerTool("pipedrive_create_deal", {
    title: "Create Deal",
...
v2 field names:
  ...
  - expected_close_date: YYYY-MM-DD`,
    inputSchema: z.object({
      ...
      currency: z.string().length(3).optional().describe("3-letter ISO currency code (use pipedrive_list_currencies)"),
      person_id: z.number().int().positive().optional().describe("Linked person ID"),
      org_id: z.number().int().positive().optional().describe("Linked organization ID"),
      pipeline_id: z.number().int().positive().optional().describe("Pipeline ID"),
      stage_id: z.number().int().positive().optional().describe("Stage ID"),
      owner_id: z.number().int().positive().optional().describe("Owner user ID"),
      ...
      label_ids: z.array(z.number().int()).optional().describe("Array of label IDs"),
      custom_fields: z.record(z.unknown()).optional().describe("Custom fields nested object"),
    }).strict(),
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
  }, async (params) => {
    try {
      const data = await client.post<SingleResponse<unknown>>("/api/v2/deals", compactBody(params as Record<string, unknown>));
      return ok(`Deal created.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  // ── UPDATE DEAL ─────────────────────────────────────────────────────────────
  server.registerTool("pipedrive_update_deal", {
    title: "Update Deal",
...
Archived deals CANNOT be edited — only is_archived=false is allowed (unarchive).`,
    inputSchema: z.object({
      ...
      is_archived: z.boolean().optional().describe("Archive (true) or unarchive (false). Archived deals CANNOT be edited for other fields (ERR_DEAL_ARCHIVED). Only is_archived=false can unarchive."),
      custom_fields: z.record(z.unknown()).optional(),
    }).strict(),
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
  }, async ({ id, ...fields }) => {
    try {
      const data = await client.patch<SingleResponse<unknown>>(`/api/v2/deals/${id}`, compactBody(fields as Record<string, unknown>));
      return ok(`Deal ${id} updated.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  // ── DELETE DEAL ─────────────────────────────────────────────────────────────
  server.registerTool("pipedrive_delete_deal", {
    title: "Delete Deal",
    ...
    try {
      ...
      return ok(`Deal ${id} deleted (soft). Permanently removed after 30 days.`);
    } catch (e) { return err(e); }
  });

  // ── SEARCH DEALS ────────────────────────────────────────────────────────────
  server.registerTool("pipedrive_search_deals", {
    title: "Search Deals",
...
Custom field types searchable: address, varchar, text, varchar_auto, double, monetary, phone.`,
    inputSchema: z.object({
      ...
      fields: z.string().optional().describe("Comma-separated fields: title, notes, custom_fields"),
      exact_match: z.boolean().optional().describe("Only exact matches (not case-sensitive)"),
      person_id: z.number().int().positive().optional().describe("Restrict to this person"),
      org_id: z.number().int().positive().optional().describe("Restrict to this org"),
    ...
    try {
      const data = await client.get<ListResponseV2<unknown>>("/api/v2/deals/search", compactBody(params as Record<string, unknown>));
      const nc = data.additional_data?.next_cursor;
      return ok(`${data.data.length} deal(s) for "${params.term}".${nc ? ` Next cursor: ${nc}` : ""}\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  // ── LIST DEAL PRODUCTS ──────────────────────────────────────────────────────
  server.registerTool("pipedrive_list_deal_products", {
...
sort_by: id | add_time | update_time | order_nr`,
    inputSchema: z.object({
      ...
      cursor: z.string().optional(),
      limit: z.number().int().min(1).max(500).default(50),
      sort_by: z.enum(["id","add_time","update_time","order_nr"]).default("id"),
      sort_direction: z.enum(["asc","desc"]).default("asc"),
    }).strict(),
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
  }, async ({ deal_id, cursor, limit, sort_by, sort_direction }) => {
    try {
      const data = await client.get<ListResponseV2<unknown>>(`/api/v2/deals/${deal_id}/products`, compactBody({ cursor, limit, sort_by, sort_direction }));
      return ok(`${data.data.length} product(s) on deal ${deal_id}.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  // ── GET PRODUCTS FOR MULTIPLE DEALS ────────────────────────────────────────
  server.registerTool("pipedrive_get_deals_products", {
...
sort_by: id | deal_id | add_time | update_time | order_nr`,
    inputSchema: z.object({
      deal_ids: z.string().describe("Comma-separated deal IDs (up to 100, e.g. '1,2,3')"),
      cursor: z.string().optional(),
      limit: z.number().int().min(1).max(500).default(50),
      sort_by: z.enum(["id","deal_id","add_time","update_time","order_nr"]).default("id"),
      sort_direction: z.enum(["asc","desc"]).default("asc"),
    }).strict(),
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
  }, async ({ deal_ids, cursor, limit, sort_by, sort_direction }) => {
    try {
      const data = await client.get<ListResponseV2<unknown>>("/api/v2/deals/products", compactBody({ deal_ids, cursor, limit, sort_by, sort_direction }));
      const nc = data.additional_data?.next_cursor;
      return ok(`${data.data.length} product line item(s).${nc ? ` Next cursor: ${nc}` : ""}\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  // ── ADD PRODUCT TO DEAL ─────────────────────────────────────────────────────
  server.registerTool("pipedrive_add_product_to_deal", {
...
is_enabled: if false, this line item is excluded from deal value calculation.`,
    inputSchema: z.object({
      deal_id: z.number().int().positive().describe("Deal ID"),
      product_id: z.number().int().positive().describe("Product ID to attach"),
      item_price: z.number().nonnegative().describe("Unit price for this deal"),
      quantity: z.number().positive().describe("Quantity"),
      discount: z.number().nonnegative().optional().describe("Discount value"),
      discount_type: z.enum(["percentage","amount"]).optional(),
      duration: z.number().positive().optional().describe("Duration (for subscriptions)"),
      ...
      is_enabled: z.boolean().optional().describe("Include in deal value calculation"),
    }).strict(),
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
  }, async ({ deal_id, ...body }) => {
    try {
      const data = await client.post<SingleResponse<unknown>>(`/api/v2/deals/${deal_id}/products`, compactBody(body as Record<string, unknown>));
      return ok(`Product added to deal ${deal_id}.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  // ── UPDATE DEAL PRODUCT ──────────────────────────────────────────────────────
  server.registerTool("pipedrive_update_deal_product", {
...
NOTE: This updates the deal-product attachment (price, qty, etc.), NOT the product catalog entry itself.`,
    inputSchema: z.object({
      deal_id: z.number().int().positive().describe("Deal ID"),
      product_attachment_id: z.number().int().positive().describe("Deal-product line item ID (from list_deal_products)"),
      item_price: z.number().nonnegative().optional().describe("Updated unit price"),
    ...
    try {
      const data = await client.patch<SingleResponse<unknown>>(`/api/v2/deals/${deal_id}/products/${product_attachment_id}`, compactBody(body as Record<string, unknown>));
      return ok(`Deal product ${product_attachment_id} updated.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  // ── DELETE DEAL PRODUCT ─────────────────────────────────────────────────────
  server.registerTool("pipedrive_delete_deal_product", {
...
product_attachment_id is the deal-product line item ID (from list_deal_products), NOT the product catalog ID.`,
    inputSchema: z.object({
      deal_id: z.number().int().positive().describe("Deal ID"),
      product_attachment_id: z.number().int().positive().describe("Deal-product line item ID"),
    }).strict(),
    annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: true },
  }, async ({ deal_id, product_attachment_id }) => {
    try {
      await client.del(`/api/v2/deals/${deal_id}/products/${product_attachment_id}`);
      return ok(`Product line item ${product_attachment_id} removed from deal ${deal_id}.`);
    } catch (e) { return err(e); }
  });

  // ── DEAL PARTICIPANTS ────────────────────────────────────────────────────────
  server.registerTool("pipedrive_list_deal_participants", {
...
Returns person objects with their contact info.`,
    ...
    try {
      const data = await client.get<ListResponseV1<unknown>>(`/api/v1/deals/${deal_id}/participants`, compactBody({ start, limit }));
      const pg = data.additional_data?.pagination;
      return ok(`${data.data?.length ?? 0} participant(s).${pg?.more_items_in_collection ? ` More (next_start: ${pg.next_start})` : ""}\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_add_deal_participant", {
...
Useful for tracking multiple contacts involved in a deal.`,
    inputSchema: z.object({
      deal_id: z.number().int().positive().describe("Deal ID"),
      person_id: z.number().int().positive().describe("Person ID to add as participant"),
    }).strict(),
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
  }, async ({ deal_id, person_id }) => {
    try {
      const data = await client.post<SingleResponse<unknown>>(`/api/v1/deals/${deal_id}/participants`, { person_id });
      return ok(`Person ${person_id} added as participant to deal ${deal_id}.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_delete_deal_participant", {
...
Use pipedrive_list_deal_participants to find the participant IDs.`,
    inputSchema: z.object({
      deal_id: z.number().int().positive().describe("Deal ID"),
      participant_id: z.number().int().positive().describe("Participant relationship ID (from list_deal_participants)"),
    }).strict(),
    annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: true },
  }, async ({ deal_id, participant_id }) => {
    try {
      await client.del(`/api/v1/deals/${deal_id}/participants/${participant_id}`);
      return ok(`Participant ${participant_id} removed from deal ${deal_id}.`);
    } catch (e) { return err(e); }
  });

  // ── DEAL FOLLOWERS ──────────────────────────────────────────────────────────
  server.registerTool("pipedrive_list_deal_followers", {
...
Returns: user_id, add_time (RFC3339).`,
    ...
    try {
      const data = await client.get<ListResponseV2<unknown>>(`/api/v2/deals/${deal_id}/followers`, compactBody({ cursor, limit }));
      const nc = data.additional_data?.next_cursor;
      return ok(`${data.data.length} follower(s).${nc ? ` Next cursor: ${nc}` : ""}\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_add_deal_follower", {
...
Returns 400 if the user is already following.`,
    ...
    try {
      const data = await client.post<SingleResponse<unknown>>(`/api/v2/deals/${deal_id}/followers`, { user_id });
      return ok(`User ${user_id} added as follower to deal ${deal_id}.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_delete_deal_follower", {
...
Use pipedrive_list_deal_followers to get user_ids.`,
  ...
  server.registerTool("pipedrive_convert_deal_to_lead", {
...
The deal is marked as deleted after successful conversion.`,
    ...
    try {
      const data = await client.post<SingleResponse<unknown>>(`/api/v2/deals/${deal_id}/convert`, {});
      return ok(`Deal-to-lead conversion initiated.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_get_deal_conversion_status", {
...
When completed, response includes the created lead ID (UUID).`,
    inputSchema: z.object({
      deal_id: z.number().int().positive().describe("Deal ID"),
      conversion_id: z.string().uuid().describe("Conversion job ID (UUID from pipedrive_convert_deal_to_lead)"),
    }).strict(),
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
  }, async ({ deal_id, conversion_id }) => {
    try {
      const data = await client.get<SingleResponse<unknown>>(`/api/v2/deals/${deal_id}/convert/status/${conversion_id}`);
      return ok(serialize(data.data));
    } catch (e) { return err(e); }
  });

  // ── LIST ARCHIVED DEALS ─────────────────────────────────────────────────────
  server.registerTool("pipedrive_list_archived_deals", {
...
Also supports same include_fields and custom_fields params.`,
    inputSchema: z.object({
      filter_id: z.number().int().positive().optional().describe("Saved filter ID"),
      ids: z.string().optional().describe("Comma-separated deal IDs (up to 100)"),
      person_id: z.number().int().positive().optional(),
      org_id: z.number().int().positive().optional(),
      pipeline_id: z.number().int().positive().optional(),
      stage_id: z.number().int().positive().optional(),
      ...
      updated_since: z.string().optional().describe("RFC3339 (e.g. 2025-01-01T10:20:00Z)"),
      updated_until: z.string().optional().describe("RFC3339"),
      cursor: z.string().optional(),
      limit: z.number().int().min(1).max(500).default(50),
      sort_by: z.enum(["id","add_time","update_time"]).default("add_time"),
      sort_direction: z.enum(["asc","desc"]).default("desc"),
    ...
    try {
      const data = await client.get<ListResponseV2<unknown>>("/api/v2/deals/archived", compactBody(params as Record<string, unknown>));
      const nc = data.additional_data?.next_cursor;
      return ok(`${data.data.length} archived deal(s).${nc ? ` Next cursor: ${nc}` : " (end)"}\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  // ── MERGE DEALS ─────────────────────────────────────────────────────────────
  server.registerTool("pipedrive_merge_deals", {
...
All participants, followers, activities, notes, files are transferred to merge_with_id.`,
    inputSchema: z.object({
      id: z.number().int().positive().describe("Deal ID that will be merged and deleted"),
      merge_with_id: z.number().int().positive().describe("Deal ID to keep (data from this deal is prioritized)"),
    }).strict(),
    annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: true },
  }, async ({ id, merge_with_id }) => {
    try {
      const data = await client.put<SingleResponse<unknown>>(`/api/v1/deals/${id}/merge`, { merge_with_id });
      return ok(`Deal ${id} merged into deal ${merge_with_id}.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  // ── DUPLICATE DEAL ──────────────────────────────────────────────────────────
  server.registerTool("pipedrive_duplicate_deal", {
...
Copies deal data, custom fields. Does NOT copy activities, notes, files, participants, or followers.`,
    ...
    try {
      const data = await client.post<SingleResponse<unknown>>(`/api/v2/deals/${id}/duplicate`, {});
