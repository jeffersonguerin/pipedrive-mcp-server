...
}).describe("Address object. v2: was flat address_* fields, now a nested object. Only 'value' is required.");

export function registerOrganizationTools(server: McpServer, client: PipedriveClient): void {

  server.registerTool("pipedrive_list_organizations", {
...
v2: address is a nested object, is_deleted replaces active_flag.`,
    inputSchema: z.object({
      ...
      ids: z.string().optional().describe("Comma-separated org IDs (up to 100)"),
      owner_id: z.number().int().positive().optional(),
      name: z.string().optional().describe("Filter organizations by name (partial match)"),
      updated_since: z.string().optional().describe("RFC3339: orgs updated at or after (e.g. 2025-01-01T10:20:00Z)"),
      updated_until: z.string().optional().describe("RFC3339: orgs updated before"),
    ...
    try {
      const data = await client.get<ListResponseV2<unknown>>("/api/v2/organizations", compactBody(params as Record<string, unknown>));
      const nc = data.additional_data?.next_cursor;
      return ok(`${data.data.length} org(s).${nc ? ` Next cursor: ${nc}` : ""}\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_get_organization", {
...
custom_fields: comma-separated keys (max 15).`,
    ...
    try {
      const data = await client.get<SingleResponse<unknown>>(`/api/v2/organizations/${id}`, compactBody({ include_fields, custom_fields }));
      return ok(serialize(data.data));
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_create_organization", {
...
visible_to: 1=Owner, 3=Company, 5=Owner's group, 7=Everyone.`,
    ...
    try {
      const data = await client.post<SingleResponse<unknown>>("/api/v2/organizations", compactBody(params as Record<string, unknown>));
      return ok(`Organization created.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_update_organization", {
...
For address: only value is required; missing subfields default to null.`,
    ...
    try {
      const data = await client.patch<SingleResponse<unknown>>(`/api/v2/organizations/${id}`, compactBody(fields as Record<string, unknown>));
      return ok(`Organization ${id} updated.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_delete_organization", {
  ...
  server.registerTool("pipedrive_search_organizations", {
...
Searchable fields: name, address, notes, custom_fields.`,
    inputSchema: z.object({
      ...
      fields: z.string().optional().describe("Fields: name, address, notes, custom_fields"),
    ...
    try {
      const data = await client.get<ListResponseV2<unknown>>("/api/v2/organizations/search", compactBody(params as Record<string, unknown>));
      const nc = data.additional_data?.next_cursor;
      return ok(`${data.data.length} org(s) for "${params.term}".${nc ? ` Next cursor: ${nc}` : ""}\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_merge_organizations", {
...
All deals, persons, activities, notes are transferred to merge_with_id.`,
    inputSchema: z.object({
      id: z.number().int().positive().describe("Organization ID that will be merged and deleted"),
      merge_with_id: z.number().int().positive().describe("Organization ID to keep (data is prioritized)"),
    }).strict(),
    annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: true },
  }, async ({ id, merge_with_id }) => {
    try {
      const data = await client.put<SingleResponse<unknown>>(`/api/v1/organizations/${id}/merge`, { merge_with_id });
      return ok(`Organization ${id} merged into ${merge_with_id}.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_list_org_followers", {
    ...
    try {
      const data = await client.get<ListResponseV2<unknown>>(`/api/v2/organizations/${org_id}/followers`, compactBody({ cursor, limit }));
      const nc = data.additional_data?.next_cursor;
      return ok(`${data.data.length} follower(s).${nc ? ` Next cursor: ${nc}` : ""}\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_add_org_follower", {
    ...
    try {
      const data = await client.post<SingleResponse<unknown>>(`/api/v2/organizations/${org_id}/followers`, { user_id });
      return ok(`User ${user_id} added as follower to org ${org_id}.\n\n${serialize(data.data)}`);
    } catch (e) { return err(e); }
  });

  server.registerTool("pipedrive_delete_org_follower", {
    ...
    try {
      await client.del(`/api/v2/organizations/${org_id}/followers/${user_id}`);
