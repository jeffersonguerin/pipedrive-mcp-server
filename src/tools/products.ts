...
const PriceSchema = z.object({
  ...
  currency: z
    .string()
    .length(3)
    .describe("3-letter currency code (use pipedrive_list_currencies)"),
...
owner_id is plain integer, product_variations moved to separate API.`,
      inputSchema: z
        .object({
          ...
          ids: z
            .string()
            .optional()
            .describe("Comma-separated product IDs (up to 100)"),
          owner_id: z
            .number()
            .int()
            .nonnegative()
          ...
          sort_by: z
            .enum(["id", "name", "add_time", "update_time"])
            .default("id"),
    ...
    async (params) => {
      try {
        ...
        return ok(
          `${data.data.length} product(s).${nc ? ` Next cursor: ${nc}` : ""}\n\n${serialize(data.data)}`,
        );
      } catch (e) {
        return err(e);
      }
      ...
      inputSchema: z
        .object({
          ...
          unit: z
            .string()
            .optional()
            .describe("Unit of measurement (e.g. pcs, hours, months)"),
          tax: z
            .number()
            .min(0)
            .max(100)
          ...
          category: z
            ...
            .int()
            .optional()
            .describe("Category ID (integer, not string)"),
          owner_id: z.number().int().positive().optional(),
          is_linkable: z
            .boolean()
            .optional()
            .describe("Can be linked to deals (default: true)"),
          visible_to: z
            .number()
            .int()
            .optional()
          ...
          prices: z
            .array(PriceSchema)
            .optional()
            .describe("Price list (one entry per currency)"),
          billing_frequency: z
            .enum([
              "one-time",
              "annually",
            ...
            .optional()
            .describe("Subscription billing frequency (Growth+ plans only)"),
          billing_frequency_cycles: z
            ...
            .optional()
            .describe(
              "Number of billing cycles (null=indefinite; must be null for one-time)",
            ),
      ...
      inputSchema: z
        .object({
          ...
          prices: z
            .array(PriceSchema)
            .optional()
            .describe("Replaces ALL existing prices"),
          billing_frequency: z
            .enum([
              "one-time",
              "annually",
...
include_fields: product.price (include price data in results).`,
      inputSchema: z
        .object({
          ...
          fields: z
            .string()
            .optional()
            .describe("Fields: name, code, custom_fields"),
    ...
    async (params) => {
      try {
        ...
        const nc = data.additional_data?.next_cursor;
        return ok(
          `${data.data.length} product(s) for "${params.term}".${nc ? ` Next cursor: ${nc}` : ""}\n\n${serialize(data.data)}`,
        );
      } catch (e) {
        return err(e);
      }
...
Returns: id, name, product_id, prices (array with currency, price, cost, direct_cost).`,
    ...
    async ({ product_id, cursor, limit }) => {
      try {
        const data = await client.get<ListResponseV2<unknown>>(
          `/api/v2/products/${product_id}/variations`,
          compactBody({ cursor, limit }),
        );
        const nc = data.additional_data?.next_cursor;
        return ok(
          `${data.data.length} variation(s).${nc ? ` Next cursor: ${nc}` : ""}\n\n${serialize(data.data)}`,
        );
      } catch (e) {
        return err(e);
      }
  ...
  server.registerTool(
    "pipedrive_create_product_variation",
    {
      ...
      inputSchema: z
        .object({
          ...
          name: z
            ...
            .min(1)
            .max(255)
            .describe("Variation name (max 255 chars, required)"),
          prices: z
            .array(
              z
                .object({
                  ...
                  currency: z
                    .string()
                    .length(3)
                    .describe("3-letter currency code"),
            ...
            .optional()
            .describe("Price list per currency"),
        })
        .strict(),
      annotations: {
        readOnlyHint: false,
...
WARNING: prices array is FULLY REPLACED when sent. Include all currencies to keep.`,
      inputSchema: z
        .object({
          ...
          prices: z
            ...
            .optional()
            .describe("Replaces ALL existing variation prices"),
        })
        .strict(),
      annotations: {
        readOnlyHint: false,
    ...
    async ({ product_id, variation_id, ...body }) => {
      try {
        const data = await client.patch<SingleResponse<unknown>>(
          `/api/v2/products/${product_id}/variations/${variation_id}`,
          compactBody(body as Record<string, unknown>),
        );
        return ok(
          `Variation ${variation_id} updated.\n\n${serialize(data.data)}`,
        );
      } catch (e) {
        return err(e);
      }
  ...
  server.registerTool(
      ...
      inputSchema: z
        .object({
          ...
          variation_id: z
            ...
            .int()
            .positive()
            .describe("Variation ID to delete"),
        })
        .strict(),
      annotations: {
        readOnlyHint: false,
    ...
    async ({ product_id, variation_id }) => {
      try {
        ...
        return ok(
          `Variation ${variation_id} deleted from product ${product_id}.`,
        );
      } catch (e) {
        return err(e);
      }
...
status: open | won | lost | deleted | all_not_deleted (default: all_not_deleted).`,
    ...
    async ({ product_id, status, start, limit }) => {
      try {
        const data = await client.get<ListResponseV1<unknown>>(
          `/api/v1/products/${product_id}/deals`,
          compactBody({ status, start, limit }),
        );
        const pg = data.additional_data?.pagination;
        return ok(
          `${data.data?.length ?? 0} deal(s) with product ${product_id}.${pg?.more_items_in_collection ? ` More (next_start: ${pg.next_start})` : ""}\n\n${serialize(data.data)}`,
        );
      } catch (e) {
        return err(e);
      }
  ...
  server.registerTool(
    ...
    async ({ product_id, cursor, limit }) => {
      try {
        const data = await client.get<ListResponseV2<unknown>>(
          `/api/v2/products/${product_id}/followers`,
          compactBody({ cursor, limit }),
        );
        const nc = data.additional_data?.next_cursor;
        return ok(
  ...
  server.registerTool(
    ...
    async ({ product_id, user_id }) => {
      try {
        ...
        return ok(
          `User ${user_id} added as follower to product ${product_id}.\n\n${serialize(data.data)}`,
        );
      } catch (e) {
        return err(e);
      }
  ...
  server.registerTool(
    ...
    async ({ product_id, user_id }) => {
      try {
        ...
        return ok(
          `User ${user_id} removed as follower from product ${product_id}.`,
        );
      } catch (e) {
        return err(e);
      }
